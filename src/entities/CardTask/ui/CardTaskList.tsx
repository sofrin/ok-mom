...
import { ListItem, ListItemButton, Skeleton, ListItemIcon, ListItemText } from '@mui/material';
...
export const CardTaskList = ({ tasks, setTasks, isLoading, setDraggableTask, draggableTask, filteredTasks, completed = false, archieved = false }: Props) => {
  ...
  return <>
    ...
    <Grid item container direction="column" xs={12} sx={{ p: 4, height: 150, overflowY: 'auto', overflowX: 'hidden', flexGrow: 0 }}>
      <List ref={parent} disablePadding>
        {!isLoading ? filteredTasks.map((obj: taskSchema) =>
          ...
          <ListItem draggable
            ...
            <ListItemButton>
              <Link className='flex items-center flex-1' to={`/Home/tasks/${obj.id}`} state={{ tasks: tasks }} >
              ...
              {completed === false && archieved === false ? <ListItemButton onClick={() => handleClickCompleted(obj)} sx={{ maxWidth: 24, padding: 0 }} >
                <DoneIcon sx={[
                  {
                    '&:hover': {
                      color: 'white',
              ...
              {completed ? <><ListItemButton onClick={() => handleClickAchive(obj)} sx={{ maxWidth: 24, padding: 0 }} >
                <ThumbUpAltIcon sx={[
                  {
                    '&:hover': {
                      color: 'green',
              ...
              </ListItemButton> <ListItemButton onClick={() => handleClickUndo(obj)} sx={{ maxWidth: 24, padding: 0 }} >
                  <UndoIcon sx={[
                    {
                      '&:hover': {
                        color: 'white',
              ...
              {archieved ? <ListItemButton onClick={() => handleClickUnArchive(obj)} sx={{ maxWidth: 25, padding: 0 }} ><UndoIcon sx={[
                {
                  '&:hover': {
                    color: 'white',
                    background: 'green'
